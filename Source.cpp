...
void display(void)
{
	...
	glRotatef(thetaSun, 0.0, rot, rot);
...
void spin(void)
{
	...
	thetaSun += (double)0.05+accelerate/360;
	printf("%lf\n", accelerate / 360);
	if (thetaSun > 360.0) thetaSun -= 360.0;

	thetaSatellite += (double)0.01+ accelerate_2/ 360;
	if (thetaSatellite > 360.0) thetaSatellite -= 360.0;

	thetaMoon += (double)0.01+ accelerate_2 / 360;
